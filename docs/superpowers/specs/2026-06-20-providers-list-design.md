...
- [settings-dialog.tsx](../../../src/components/settings-dialog.tsx) فيه دالة `applyPreset` (3 مزوّدات) **غير مستخدَمة إطلاقًا** في الواجهة، وبروكسي قديم `https://ai.huan666.de/v1` داخل `TEST_CONFIG`.
...
4. تنظيف الكود القديم (`applyPreset`، بروكسي `ai.huan666.de`).
...
- إزالة بروكسي `https://ai.huan666.de/v1`: حذف كائن `TEST_CONFIG` ودالة `loadTestConfig` وزرّ «testPreset» (يُغني عنها منتقي المزوّد). وضع `demo` (apiKey = `'demo'`) يبقى عاملًا عبر كتابة المفتاح يدويًّا (منطقه في `chat-client.ts` لا يتغيّر).
